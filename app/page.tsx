    ...
    <div className="min-h-screen bg-[#0d0d0d] text-white">
      ...
      <div className="text-center px-6 pt-24 pb-14">
        <div className="inline-block bg-[#1a1a00] text-[#f5a623] border border-[#3a3000] text-xs px-4 py-2 rounded-full mb-8">
          {lang === "it" ? "Per piccole aziende di trasporto · 1-20 camion" : "Pentru firme mici de transport · 1-20 camioane"}
        </div>
        <h1 className="text-6xl font-semibold leading-tight mb-6 max-w-3xl mx-auto text-white">
          {lang === "it" ? <>In 10 secondi sai se<br />un viaggio <span className="text-[#f5a623]">vale o no.</span></> : <>În 10 secunde știi dacă<br />o cursă <span className="text-[#f5a623]">merită sau nu.</span></>}
        </h1>
        <p className="text-xl text-gray-300 mb-4 max-w-xl mx-auto">
          {lang === "it" ? "Inserisci i dati del viaggio. TripProfit calcola tutto e ti dice subito: ACCETTA, NEGOZIA o RIFIUTA." : "Introduci datele cursei. TripProfit calculează tot și îți spune direct: ACCEPTĂ, NEGOCIAZĂ sau REFUZĂ."}
        </p>
        <p className="text-sm text-gray-500 mb-10 max-w-md mx-auto">
          {lang === "it" ? "Non è un software complesso. È uno strumento che calcola la redditività reale di ogni viaggio." : "Nu este un software complex. Este un instrument care calculează profitabilitatea reală a fiecărei curse."}
        </p>
        <div className="flex items-center justify-center gap-4 mb-4">
          <Link href="/register" className="bg-[#f5a623] text-black font-semibold px-8 py-4 rounded-lg hover:bg-[#e8951a] transition text-base">
            {lang === "it" ? "Prova 30 giorni gratis" : "Încearcă 30 zile gratuit"}
          </Link>
          <Link href="/login" className="border border-[#3a3a3a] text-gray-300 px-8 py-4 rounded-lg hover:text-white hover:border-[#555] transition text-base">
            {lang === "it" ? "Accedi" : "Intră în cont"}
          </Link>
        </div>
        <p className="text-xs text-gray-600">{lang === "it" ? "Senza carta di credito. Senza impegni." : "Fără card bancar. Fără angajamente."}</p>
      </div>

      {/* DEMO VERDICT */}
      <div className="max-w-md mx-auto px-6 mb-24">
        <div className="bg-[#0a1f0a] border border-green-900 rounded-xl p-6 text-center">
          <div className="text-sm text-gray-500 mb-3">{lang === "it" ? "Viaggio București → München · 1.200 km · 1.850€" : "Cursă București → München · 1.200 km · 1.850€"}</div>
          <div className="text-5xl font-semibold text-green-400 mb-2">+482 €</div>
          <div className="text-2xl font-semibold text-green-400 mb-3">{lang === "it" ? "ACCETTA" : "ACCEPTĂ"}</div>
          <div className="text-sm text-gray-500">{lang === "it" ? "1.54 €/km · sopra la soglia minima impostata da te" : "1.54 €/km · peste pragul minim setat de tine"}</div>
        </div>
      </div>
      ...
      <div className="border-t border-b border-[#1e1e1e] bg-[#111] py-7 px-6 text-center mb-24">
        <p className="text-base text-gray-300 max-w-lg mx-auto">
          {lang === "it" ? <>Costruito per le piccole aziende di trasporto che gestiscono viaggi internazionali e hanno bisogno di chiarezza finanziaria rapida.{" "}<strong className="text-white">TripProfit calcola prima che appaiano le perdite.</strong></> : <>Construit pentru firmele mici de transport care gestionează curse internaționale și au nevoie de claritate financiară rapidă.{" "}<strong className="text-white">TripProfit calculează înainte să apară pierderile.</strong></>}
        </p>
      </div>

      {/* PROBLEMA */}
      <div className="max-w-4xl mx-auto px-6 mb-24 text-center">
        <div className="text-xs text-[#f5a623] uppercase tracking-widest mb-5">{lang === "it" ? "Il problema" : "Problema"}</div>
        <h2 className="text-4xl font-semibold mb-4 text-white">{lang === "it" ? "Perché le piccole aziende di trasporto perdono soldi?" : "De ce pierd bani firmele mici de transport?"}</h2>
        <p className="text-lg text-gray-400 mb-14">{lang === "it" ? "Non per mancanza di lavoro. Ma per mancanza di una visione chiara sui costi reali." : "Nu din lipsă de muncă. Ci din lipsa unei imagini clare asupra costurilor reale."}</p>
      ...
      <div id="functii" className="bg-[#0a0a0a] border-t border-b border-[#1e1e1e] py-24 px-6 mb-24">
        <div className="max-w-4xl mx-auto text-center">
          <div className="text-xs text-[#f5a623] uppercase tracking-widest mb-5">{lang === "it" ? "Funzionalità" : "Funcționalități"}</div>
          <h2 className="text-4xl font-semibold mb-4 text-white">{lang === "it" ? "Cosa fa TripProfit per la tua azienda" : "Ce face TripProfit pentru firma ta"}</h2>
          <p className="text-lg text-gray-400 mb-14">{lang === "it" ? "Tutte le informazioni rilevanti, disponibili immediatamente, senza cercarle tu." : "Toate informațiile relevante, disponibile imediat, fără să le cauți tu."}</p>
      ...
      <div id="preturi" className="max-w-4xl mx-auto px-6 mb-24 text-center">
        <div className="text-xs text-[#f5a623] uppercase tracking-widest mb-5">{lang === "it" ? "Prezzi" : "Prețuri"}</div>
        <h2 className="text-4xl font-semibold mb-4 text-white">{lang === "it" ? "Chiaro e prevedibile." : "Clar și previzibil."}</h2>
        <p className="text-lg text-gray-400 mb-4">{lang === "it" ? "30 giorni gratis per qualsiasi piano. Senza carta di credito alla registrazione." : "30 de zile gratuit pentru orice plan. Fără card bancar la înregistrare."}</p>
        <p className="text-sm text-gray-600 mb-14">
          {lang === "it" ? "Dopo il periodo di prova, scegli il piano adatto alla tua azienda e lo attivi direttamente dall'app, nella sezione Prezzi." : "După perioada de test, alegi planul potrivit firmei tale și îl activezi direct din aplicație, din secțiunea Prețuri."}
        </p>
        <div className="grid grid-cols-3 gap-5">
          <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-8 text-left">
            ...
            <div className="text-5xl font-semibold text-white mb-1"><sup className="text-xl">€</sup>30<sub className="text-sm font-normal text-gray-500">{lang === "it" ? "/mese" : "/lună"}</sub></div>
            <div className="text-sm text-gray-400 mb-6 mt-2">{lang === "it" ? "Per le aziende che vogliono eliminare i viaggi non redditizi" : "Pentru firmele care vor să elimine cursele neprofitabile"}</div>
            <div className="border-t border-[#2a2a2a] pt-5 mb-7 space-y-2.5">
              {(lang === "it" ? ["Calcolatore viaggio con verdetto istantaneo", "Costo reale per km", "1 camion", "Storico viaggi"] : ["Calculator cursă cu verdict instant", "Cost real per km", "1 camion", "Istoric curse"]).map(f => (
                <div key={f} className="flex gap-2 text-sm text-gray-300"><span className="text-green-400">✓</span>{f}</div>
              ))}
              {(lang === "it" ? ["Report per camion", "Report per cliente", "Cashflow tracking"] : ["Raport per camion", "Raport per client", "Cashflow tracking"]).map(f => (
            ...
            <Link href="/register" className="block text-center border border-[#333] text-white py-3 rounded-lg text-sm hover:bg-[#1e1e1e] transition">
              {lang === "it" ? "Inizia gratis" : "Începe gratuit"}
            </Link>
          </div>
          ...
          <div className="bg-[#16143a] border-2 border-[#4f46e5] rounded-xl p-8 text-left relative">
            <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#f5a623] text-black text-xs font-semibold px-4 py-1 rounded-full whitespace-nowrap">
              {lang === "it" ? "Il più scelto" : "Cel mai ales"}
            </div>
            <div className="text-xs text-gray-500 uppercase tracking-wider mb-4">Pro</div>
            <div className="text-5xl font-semibold text-white mb-1"><sup className="text-xl">€</sup>49<sub className="text-sm font-normal text-gray-500">{lang === "it" ? "/mese" : "/lună"}</sub></div>
            <div className="text-sm text-gray-400 mb-6 mt-2">{lang === "it" ? "Per le aziende che vogliono visibilità completa sulle finanze" : "Pentru firmele care vor vizibilitate completă asupra finanțelor"}</div>
            <div className="border-t border-[#2a2a2a] pt-5 mb-7 space-y-2.5">
              {(lang === "it" ? ["Tutto di Basic", "Camion illimitati", "Report per camion", "Report per cliente con punteggio rischio", "Dashboard generale azienda", "Avvisi automatici costi superati"] : ["Tot ce include Basic", "Camioane nelimitate", "Raport per camion", "Raport per client cu scor de risc", "Dashboard general firmă", "Alerte automate costuri depășite"]).map(f => (
                <div key={f} className="flex gap-2 text-sm text-gray-300"><span className="text-green-400">✓</span>{f}</div>
              ))}
              {(lang === "it" ? ["Cashflow tracking"] : ["Cashflow tracking"]).map(f => (
                <div key={f} className="flex gap-2 text-sm text-gray-600"><span>✗</span>{f}</div>
              ))}
            </div>
          ...
          <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-8 text-left">
            ...
            <div className="text-5xl font-semibold text-white mb-1"><sup className="text-xl">€</sup>79<sub className="text-sm font-normal text-gray-500">{lang === "it" ? "/mese" : "/lună"}</sub></div>
            <div className="text-sm text-gray-400 mb-6 mt-2">{lang === "it" ? "Per le aziende che vogliono il controllo finanziario completo" : "Pentru firmele care vor control financiar complet"}</div>
            <div className="border-t border-[#2a2a2a] pt-5 mb-7 space-y-2.5">
              {(lang === "it" ? ["Tutto di Pro", "Cashflow tracking", "Avviso blocco liquidità", "Punteggio rischio dettagliato per cliente", "Simulazioni finanziarie", "Raccomandazioni automatiche", "Assistenza prioritaria"] : ["Tot ce include Pro", "Cashflow tracking", "Alertă blocaj de lichiditate", "Scor de risc detaliat per client", "Simulări financiare", "Recomandări automate", "Asistență prioritară"]).map(f => (
                <div key={f} className="flex gap-2 text-sm text-gray-300"><span className="text-green-400">✓</span>{f}</div>
              ))}
            </div>
            <Link href="/register" className="block text-center border border-[#333] text-white py-3 rounded-lg text-sm hover:bg-[#1e1e1e] transition">
        ...
        <div className="mt-8 bg-[#161616] border border-[#2a2a2a] rounded-xl p-5 max-w-xl mx-auto text-left">
          <div className="text-sm font-semibold text-white mb-2">{lang === "it" ? "Come attivi il piano dopo il periodo di prova?" : "Cum activezi planul după perioada de test?"}</div>
          <div className="text-sm text-gray-400 leading-relaxed">
            {lang === "it" ? <>Alla fine dei 30 giorni gratuiti, entri nell'app nella sezione <strong className="text-white">Prezzi</strong> e selezioni il piano adatto. L'attivazione richiede meno di un minuto. I tuoi dati rimangono intatti indipendentemente dal piano scelto.</> : <>La finalul celor 30 de zile gratuite, intri în aplicație la secțiunea <strong className="text-white">Prețuri</strong> și selectezi planul potrivit. Activarea durează mai puțin de un minut. Datele tale rămân intacte indiferent de planul ales.</>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0a0a0a] border-t border-[#1e1e1e] py-24 px-6 text-center">
        <h2 className="text-4xl font-semibold mb-5 text-white">
          {lang === "it" ? <>Pronto ad avere il controllo<br />della tua azienda?</> : <>Gata să ai control<br />asupra firmei tale?</>}
        </h2>
        <p className="text-lg text-gray-400 mb-10">{lang === "it" ? "Configuri l'azienda in due minuti. Il primo viaggio calcolato subito." : "Configurezi firma în două minute. Prima cursă calculată imediat."}</p>
        ...
        <p className="text-sm text-gray-600 mt-5">{lang === "it" ? "Senza carta di credito. Senza impegni. Puoi annullare in qualsiasi momento." : "Fără card bancar. Fără angajamente. Poți anula oricând."}</p>
      </div>
      ...
      <footer className="border-t border-[#1e1e1e] py-6 px-10 flex items-center justify-between">
        ...
        <div className="flex gap-6 text-sm text-gray-600">
          <Link href="/terms" className="hover:text-gray-400">{lang === "it" ? "Termini e condizioni" : "Termeni și condiții"}</Link>
          <Link href="/privacy" className="hover:text-gray-400">{lang === "it" ? "Politica sulla privacy" : "Politica de confidențialitate"}</Link>
