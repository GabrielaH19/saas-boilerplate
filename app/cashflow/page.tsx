...
export default function CashflowPage() {
  ...
  const handleSave = async () => {
    if (!userId) return;
    if (!form.clientName || !form.amount || !form.dueDate) return alert("Completează client, sumă și dată scadență.");
    setSaving(true);
    try {
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {totalOverdue > 0 && (
          <div className="bg-red-900 bg-opacity-30 border border-red-800 rounded-xl px-5 py-4 mb-6">
            ...
            <div className="text-sm text-gray-400">Ai {totalOverdue.toLocaleString()}€ {tr.overdueDesc}</div>
          </div>
        )}
              ...
              return (
                <div key={inv.id} className="grid grid-cols-6 gap-4 px-5 py-4 border-b border-[#1e1e1e] last:border-0 items-center hover:bg-[#1a1a1a] transition">
                  ...
                  <div>
                    ...
                    {inv.paidAmount > 0 && inv.paidAmount < inv.amount && <div className="text-xs text-gray-500">{tr.collected}: {inv.paidAmount.toLocaleString()}€</div>}
                  </div>
                  <div>
                    ...
                    <div className={`text-xs mt-0.5 ${days < 0 ? "text-red-400" : days <= 7 ? "text-[#f5a623]" : "text-gray-500"}`}>
                      {days < 0 ? `${Math.abs(days)} ${tr.daysOverdue}` : days === 0 ? tr.dueToday : `${tr.daysLeft} ${days} zile`}
                    </div>
                  </div>
                  <div>
                    <span className={`text-xs px-2 py-1 rounded border font-semibold ${statusStyle(inv.status)}`}>{statusLabel(inv.status)}</span>
                  </div>
                  <div className="flex gap-2 justify-end">
                    {inv.status !== "paid" && (
                      <button onClick={() => handleMarkPaid(inv)} className="text-xs text-green-400 hover:text-green-300 border border-green-800 px-2 py-1 rounded">{tr.markPaid}</button>
                    )}
                    <button onClick={() => handleEdit(inv)} className="text-xs text-gray-400 hover:text-white border border-[#2e2e2e] px-2 py-1 rounded">Edit</button>
                    <button onClick={() => handleDelete(inv.id)} className="text-xs text-red-400 hover:text-red-300 border border-[#2e2e2e] px-2 py-1 rounded">×</button>
                  </div>
                </div>
              );
            })}
          ...
          <div className="bg-[#161616] border border-[#f5a623] rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div className="col-span-2">
                ...
                <input className={inp} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="ex: Cursă București-München" />
              </div>
            </div>
